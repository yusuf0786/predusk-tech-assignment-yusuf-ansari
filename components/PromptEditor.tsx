...
export const PromptEditor: React.FC = () => {
  ...
  const send = () => {
    ...
    // mocked response
    const reply = `Model: ${modelId ?? '—'} | temp=${temperature}\n\nThis is a mocked response for: ${prompt.slice(0,120)}`;
    ...
    <div className="rounded-2xl bg-white p-4 shadow dark:bg-gray-900">
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="secondary" onClick={()=>setOpen(true)}>Templates</Button>
          <Button onClick={send}>Send ▷</Button>
        </div>
      </div>
      <textarea value={prompt} onChange={(e)=>setPrompt(e.target.value)} placeholder="Type your prompt…" aria-label="Prompt editor"
      ...
      <Modal open={open} onClose={()=>setOpen(false)} title="Templates">
        <div className="mb-3"><input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Search…" className="w-full rounded-xl border p-2" /></div>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {filtered.map(t => (
            <div key={t.id} className="rounded-xl border p-3">
              ...
              <div className="flex gap-2">
                <Button variant="secondary" onClick={()=>{ setPrompt(t.prompt); setOpen(false); }}>Load</Button>
              </div>
            </div>
          ))}
        </div>
